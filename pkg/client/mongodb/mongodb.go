...
func NewClient(ctx context.Context, host, port, username, password, database, authDB string) (db *mongo.Database, err error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("failed to connect to mongoDB due to error %v", err))
	}
	// Ping

	if err = client.Ping(ctx, nil); err != nil {
		return nil, errors.New(fmt.Sprintf("failed to ping mongoDB due to error %v", err))
	}
